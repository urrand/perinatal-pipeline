...
[ $# -eq 3 ] || { echo "usage: $(basename "$0") <subject>" 1>&2; exit 1; }
subj=$1
type=$2
atname=$3
...
atlas=${atname}"_"$atnum
if [ ! -f dofs/$subj-$atlas-n_1Warp.nii.gz ];then continue;fi

#transform atlas labels
if [ ! -f $sdir/transformations/$subj-$atlas.nii.gz ];then
${ANTSPATH}/antsApplyTransforms -d 3 -i $DRAWEMDIR/atlases/${atname}s/segmentations-v3/$atlas.nii.gz -r N4/$subj.nii.gz -o $sdir/transformations/$subj-$atlas.nii.gz -n NearestNeighbor -t [dofs/$subj-$atlas-n_1Warp.nii.gz] --verbose 1 -f 0
#ms=$sdir/template/$str/$subj.nii.gz
#run mirtk transform-image $DRAWEMDIR/atlases/${atname}s/segmentations-v3/$atlas.nii.gz $sdir/transformations/$subj-$atlas.nii.gz -target N4/$subj.nii.gz -dofin dofs/$subj-$atlas-n.dof.gz -interp NN 
fi
if [ ! -f $sdir/transformations/tissues-$subj-$atlas.nii.gz ];then 
${ANTSPATH}/antsApplyTransforms -d 3 -i $DRAWEMDIR/atlases/${atname}s/tissues-v3/$atlas.nii.gz -r N4/$subj.nii.gz -o $sdir/transformations/tissues-$subj-$atlas.nii.gz -n NearestNeighbor -t [dofs/$subj-$atlas-n_1Warp.nii.gz] --verbose 1 -f 0
#ms=$sdir/template/$str/$subj.nii.gz
#run mirtk transform-image $DRAWEMDIR/atlases/${atname}s/tissues-v3/$atlas.nii.gz $sdir/transformations/tissues-$subj-$atlas.nii.gz -target N4/$subj.nii.gz -dofin dofs/$subj-$atlas-n.dof.gz -interp NN 
fi

#transform atlases
if [ ! -f $sdir/transformations/$type-$subj-$atlas.nii.gz ];then 
${ANTSPATH}/antsApplyTransforms -d 3 -i $DRAWEMDIR/atlases/${atname}s/$type/$atlas.nii.gz -r N4/$subj.nii.gz -o $sdir/transformations/$type-$subj-$atlas.nii.gz -n BSpline -t [dofs/$subj-$atlas-n_1Warp.nii.gz] --verbose 1 -f 0
#ms=$sdir/template/$str/$subj.nii.gz
#run mirtk transform-image $DRAWEMDIR/atlases/${atname}s/$type/$atlas.nii.gz $sdir/transformations/$type-$subj-$atlas.nii.gz -target N4/$subj.nii.gz -dofin dofs/$subj-$atlas-n.dof.gz -interp BSpline 
fi

#weight atlas locally
if [ ! -f $sdir/atlas-weights/$subj-$atlas.nii.gz ];then 
run mirtk normalize N4/$subj.nii.gz $sdir/transformations/$type-$subj-$atlas.nii.gz $sdir/atlas-weights/$subj-$atlas-normalized.nii.gz -piecewise 
